...
// Vector is a simple struct that can carry a circular or linear DNA sequence.
type Vector struct {
	Sequence string
	Circular bool
}
...
func CutWithEnzymeByName(seq Vector, directional bool, enzymeStr string) ([]Fragment, error) {
...
func CutWithEnzyme(seq Vector, directional bool, enzyme Enzyme) []Fragment {
...
func recurseLigate(wg *sync.WaitGroup, c chan string, seedFragment Fragment, fragmentList []Fragment) {
	...
	if seedFragment.ForwardOverhang == seedFragment.ReverseOverhang {
		...
		for _, newFragment := range fragmentList {
			...
			if (seedFragment.ReverseOverhang == reverse.Complement(newFragment.ReverseOverhang)) && (seedFragment.ReverseOverhang != reverse.Complement(seedFragment.ReverseOverhang)) { // If the second statement isn't there, program will crash on palindromes
				newSeed := Fragment{seedFragment.Sequence + seedFragment.ReverseOverhang + reverse.Complement(newFragment.Sequence), seedFragment.ForwardOverhang, reverse.Complement(newFragment.ForwardOverhang)}
				wg.Add(1)
				go recurseLigate(wg, c, newSeed, fragmentList)
			}
		}
...
func getConstructs(c chan string, constructSequences chan []Vector) {
	var constructs []Vector
	var exists bool
	...
	for {
		...
		if more {
			...
			if !exists {
				constructs = append(constructs, Vector{construct, true})
...
func GoldenGate(sequences []Vector, enzymeStr string) ([]Vector, error) {
	var fragments []Fragment
