...
func dotBracketFromSecondaryStructure(secondaryStructure SecondaryStructure) string {
	...
	return dotBracketFromStructures(secondaryStructure.Structures, &dotBracket)
}
...
func dotBracketFromStem(dotBracket *[]byte, stem Stem) {
	(*dotBracket)[stem.ClosingFivePrimeIdx] = dotBracketStemFivePrimeNucleotide
	(*dotBracket)[stem.ClosingThreePrimeIdx] = dotBracketStemThreePrimeNucleotide
	for _, stemStructure := range stem.Structures {
		dotBracketFromStemStructure(dotBracket, stemStructure)
	}
}

func dotBracketFromStemStructure(dotBracket *[]byte, stemStructure StemStructure) {
	for i := stemStructure.ClosingFivePrimeIdx + 1; i < stemStructure.EnclosedFivePrimeIdx; i++ {
		(*dotBracket)[i] = dotBracketUnpairedNucleotide
	}
	(*dotBracket)[stemStructure.EnclosedFivePrimeIdx] = dotBracketStemFivePrimeNucleotide

	for i := stemStructure.EnclosedThreePrimeIdx + 1; i < stemStructure.ClosingThreePrimeIdx; i++ {
		(*dotBracket)[i] = dotBracketUnpairedNucleotide
	}
	(*dotBracket)[stemStructure.EnclosedThreePrimeIdx] = dotBracketStemThreePrimeNucleotide
}
